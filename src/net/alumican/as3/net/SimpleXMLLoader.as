package net.alumican.as3.net
{
	...
	public class SimpleXMLLoader extends EventDispatcher
	{
		...
		/**
		 * UTF-8 エンコーディングの文字コードを表します．
		 */
		static public const UTF_8:String = "utf-8";
		
		/**
		 * Shift-JIS エンコーディングの文字コードを表します．
		 */
		static public const SHIFT_JIS:String = "shift-jis";
		
		/**
		 * EUC-JP エンコーディングの文字コードを表します．
		 */
		static public const EUC_JP:String = "euc-jp";
		
		/**
		 * JIS エンコーディングの文字コードを表します．
		 */
		static public const JIS:String = "iso-2022-jp";
		...
		/**
		 * XMLを読み込みます．
		 * @param	url		読み込むxmlのURL
		 * @param	charset	xmlの文字コード
		 * @return
		 */
		public function load(url:String, charset:String = "utf-8"):URLLoader
		{
			_charset = charset;
			
			var loader:URLLoader = new URLLoader();
			
			loader.addEventListener(Event.COMPLETE, _loadCompleteHandler);
			loader.addEventListener(IOErrorEvent.IO_ERROR, _loadIOErrorHandler);
			loader.addEventListener(SecurityErrorEvent.SECURITY_ERROR, _loadSecurityErrorHandler);
			
			loader.dataFormat = (_charset == UTF_8) ? URLLoaderDataFormat.TEXT : URLLoaderDataFormat.BINARY;
		...
		private function _loadCompleteHandler(e:Event):void 
		{
			...
			loader.removeEventListener(Event.COMPLETE, _loadCompleteHandler);
			loader.removeEventListener(IOErrorEvent.IO_ERROR, _loadIOErrorHandler);
			loader.removeEventListener(SecurityErrorEvent.SECURITY_ERROR, _loadSecurityErrorHandler);
			...
			else
			{
				...
				{
					//euc-jp
					_xml = new XML( Jcode.EUCtoUTF8(ba) );
				}
				else
				{
					//それ以外
					_xml = new XML( ba.readMultiByte(ba.length, _charset) );
				}
			}
			
			//コールバック関数の呼び出し
			if (onLoadComplete != null) onLoadComplete(_xml);
