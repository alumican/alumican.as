package net.alumican.draft.as3.al3d.objects.primitives
{
	...
	public class Sphere extends Object3D
	{
		 ...
		 * コンストラクタ
		 */
		public function Sphere(radius:Number, texture:BitmapData = null, segmentW:uint = 8, segmentH:uint = 6):void
		{
			_radius   = radius;
			_segmentW = segmentW;
			_segmentH = segmentH;
			...
			for (i = 0; i < n; ++i)
			{
				...
				for (j = 0; j < 4; ++j)
				{
					...
					if ((p = poly[j]).index == -1)
					{
						...
						uvts.push(p.u, p.v, null);
						
						uvts2.push(p.u, p.v);
						triangleUvts.push(p.u, p.v);
						
						vertices2D.push(p.u, p.v);
					}
				}
